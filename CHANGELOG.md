...
- 数字山水知识图谱支持手动拖动单个节点；拖动时关联线实时跟随，松手后位置会在当前图谱页面本地保留。
- 新增“恢复布局”按钮，可清空手动调整并回到自动布局。
- 新增节点拖动回归测试和真实页面验证，覆盖拖动、刷新保留、恢复布局和点击阅读不互相干扰。

### 改进

- 图谱本地状态按 wiki 名称和页面路径隔离；图谱内容变化后会自动忽略旧的手动位置。
